...
static void M_Banner( char *name )
{
	...
	re.DrawPic( viddef.width / 2 - w / 2, viddef.height / 2 - 110, name, 1 );
}
...
const char *Default_MenuKey( menuframework_s *m, int key )
{
	...
	switch ( key )
	{
	case K_MOUSE2: //mouse support in menu -Maniac
	case K_ESCAPE:
		M_PopMenu();
...
{
	re.DrawChar ( cx + ((viddef.width - 320)>>1), cy + ((viddef.height - 240)>>1), num, FC_WHITE, 1);
}
...
{
	re.DrawPic (x + ((viddef.width - 320)>>1), y + ((viddef.height - 240)>>1), pic, 1);
}
...
void M_Main_Draw (void)
{
	...
	{
		if ( i != m_main_cursor )
			re.DrawPic( xoffset, ystart + i * 40 + 13, names[i], 1 );
	}
	strcpy( litname, names[m_main_cursor] );
	strcat( litname, "_sel" );
	re.DrawPic( xoffset, ystart + m_main_cursor * 40 + 13, litname, 1 );

	M_DrawCursor( xoffset - 25, ystart + m_main_cursor * 40 + 11, (int)(cls.realtime / 100)%NUM_CURSOR_FRAMES );

	re.DrawGetPicSize( &w, &h, "m_main_plaque" );
	re.DrawPic( xoffset - 30 - w, ystart, "m_main_plaque", 1 );

	re.DrawPic( xoffset - 30 - w, ystart + h + 5, "m_main_logo", 1 );
}
...
const char *M_Main_Key (int key)
{
	...
	switch (key)
	{
	case K_MOUSE2:  //Mouse support in menus -Maniac
	case K_ESCAPE:
		M_PopMenu ();
		break;
...
{
	if ( bind_grab )
		re.DrawChar( menu->x, menu->y + menu->cursor * 9, '=', FC_WHITE, 1 );
	else
		re.DrawChar( menu->x, menu->y + menu->cursor * 9, 12 + ( ( int ) ( Sys_Milliseconds() / 250 ) & 1 ), FC_WHITE, 1 );
}
...
*/
	...
	s_options_new_options_action.generic.y		= 140;
	...
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_new_options_action ); //Added new options -Maniac
...
static menulist_s		s_options_console_action2;
...
void Options_MenuInit2( void )
{
	...
	*/
	...
	s_options_console_action2.generic.type		= MTYPE_ACTION;
	s_options_console_action2.generic.x			= 0;
	s_options_console_action2.generic.y			= y += 10;
	s_options_console_action2.generic.name		= "go to console";
	s_options_console_action2.generic.callback	= ConsoleFunc;
	s_options_console_action2.generic.statusbar	= "Go back to the Console";
	...
	for (i=0 ; i<OPTS_TOTAL ; i++)
	{
		Menu_AddItem( &s_options_menu2[i], ( void * ) &s_options_console_action2 );
	}
}

void Options_MenuDraw2 (void)
{
	M_Banner( "m_banner_options" );
	Menu_AdjustCursor( &s_options_menu2[curmenu], 1 );
	Menu_Draw( &s_options_menu2[curmenu] );
}

const char *Options_MenuKey2( int key )
{
	return Default_MenuKey( &s_options_menu2[curmenu], key );
}

void M_Menu_NewOptions_f (void)
{
	Options_MenuInit2();
	M_PushMenu ( Options_MenuDraw2, Options_MenuKey2 );
}

/**************************
 *	New options menu end
 *************************/
	...
	*/
	for ( i = 0, y = viddef.height - ( ( cls.realtime - credits_start_time ) / 40.0F ); credits[i] && y < viddef.height; y += 10, i++ )
	{
		...
		for ( j = 0; credits[i][j+stringoffset]; j++ )
		{
			...
			if ( bold )
				re.DrawChar( x, y, credits[i][j+stringoffset] + 128, FC_WHITE, 1 );
			else
				re.DrawChar( x, y, credits[i][j+stringoffset], FC_WHITE, 1 );
		}
	}
...
void PlayerConfig_MenuDraw( void )
{
	...
	if ( s_pmi[s_player_model_box.curvalue].skindisplaynames )
	{
		...
		re.DrawPic( s_player_config_menu.x - 40, refdef.y, scratch, 1 );
	}
}
...
void M_Quit_Draw (void)
{
	...
	re.DrawPic ( (viddef.width-w)/2, (viddef.height-h)/2, "quit", 1 );
}
...
void M_Init (void)
{
	...
	Cmd_AddCommand ("menu_newoptions", M_Menu_NewOptions_f); //Added command to new options menu -Maniac
